<?php 

function partials_footer(){
?>   
<style>
.list-group-item{
    background-color: rgba(0,0,0,0);
}
</style>
    <footer class="footer">
        <div class="">
            <div class="row ">
                <div class="col-2 col-md-7 col-lg-5 col-xl-3">
                    <ul class="list-group list-group-flush social-links">
                    <li class="list-group-item">
                        <a href="https://www.youtube.com/channel/UC87j8G2WRUQur4VCHjF3OKQ" target="_blank" ><img src="assets/img/logo-youtube2.png"><p class="sm-hide">Chaîne Officielle de la Fédération</p></a>
                    </li>
                    <li class="list-group-item">
                        <a href="https://www.facebook.com/FederationVovinamVietVoDaoFrance" target="_blank"><img src="assets/img/fb.png"><p class="sm-hide">Page Facebook Officielle</p></a>
                    </li>
                    <li class="list-group-item">
                        <a href="https://www.instagram.com/federationvovinamvietvodaofr/" target="_blank" ><img src="assets/img/insta.png"><p class="sm-hide">Compte Instagram Officiel</p></a>
                    </li>
                    </ul>
                </div>
                <div class="col-6 only-md-hide col-lg-4 col-xl-3 text-center">
                    <h5>Fédération VOVINAM-Viet Vo Dao France</h5>
                    <p><i class="fas fa-map-marker-alt"></i> 6 allée des Agrions 91160, Ballainvilliers – France</p>
                    <p>Directeur de la publication : Amar GUERRIB</p>
                </div>
                <div class="lg-hide col-xl-4 text-center">
                    <h5>Hébergeur: GANDI SAS</h5>
                    <p>SAS au capital de 2 300 000 €</p>
                    <p>RCS Paris 423 093 459</p>
                    <p>Code APE 7740Z</p>
                    <p>N° TVA : FR 81423093459</p>
                    <p><i class="fas fa-map-marker-alt"></i> 63-65 boulevard de Masséna – France</p>
                    <p>Tél: 01.70.37.76.61</p>
                </div>
                <div class="col-3 col-md-5 col-lg-3 col-xl-2 text-center">
                    <a href="http://vietvodao.photos/" target="_blank" ><img src="assets/img/galerie.png" width=150px></a>
                </div>
                <div class="sm-hide col-md-12 only-lg-hide only-xl-hide text-center">
                    <hr>
                    <h5>Fédération VOVINAM-Viet Vo Dao France</h5>
                    <p><i class="fas fa-map-marker-alt"></i> 6 allée des Agrions 91160, Ballainvilliers – France</p>
                    <p>Directeur de la publication : Amar GUERRIB</p>
                    <!--
                    <br>
                    <h5>Hébergeur: GANDI SAS</h5>
                    <h5>SAS au capital de 2 300 000 €</h5>
                    <h5>RCS Paris 423 093 459</h5>
                    <h5>Code APE 7740Z</h5>
                    <h5>N° TVA : FR 81423093459</h5>
                    <h5>Siège social : 63-65 boulevard de Masséna – France</h5>
                    <h5>Tél: 01.70.37.76.61</h5>-->
                </div>
            </div>
        </div>
    </footer>
</body>
<?php
}
?>